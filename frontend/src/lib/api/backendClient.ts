...
export interface WcagCheckResponse {
  response: string
  model?: string
}
...
export class BackendClient {
  ...
  async requestSummary(
    ...
    try {
      console.log(
        "VIX: Enviando requisição para:",
        `${this.config.baseUrl}/summarize_page`
      )
      console.log("VIX: Payload:", {
      ...
      try {
        ...
        throw new Error(
          `Resposta não é JSON válido: ${responseText.substring(0, 100)}`
        )
      }
      ...
      throw new Error(
        `Erro na geração de resumo: ${error instanceof Error ? error.message : "Erro desconhecido"}`
      )
    }
  }
  ...
  async requestImageAlt(
    ...
    const startTime = Date.now()
    console.log("VIX: ###WILL REQUES IMAGE ALT###")
    try {
      ...
      console.log("VIX: ###WILL REQUES IMAGE ALT###RESPONSE CHEGOU")
      if (!response.ok) {
      ...
      throw new Error(
        `Erro na geração de alt text: ${error instanceof Error ? error.message : "Erro desconhecido"}`
      )
    }
  }

  async requestWCAGCheck(content: string): Promise<WcagCheckResponse> {
    const startTime = Date.now()
    try {
      const response = await fetch(`${this.config.baseUrl}/wcag_check`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json"
      ...
      console.error("VIX: WCAG check error:", error)
      throw new Error(
        `WCAG check failed: ${error instanceof Error ? error.message : "Unknown error"}`
      )
    }
  }
...
// Instância singleton do cliente usando variáveis de ambiente
export const backendClient = new BackendClient({
  baseUrl: process.env.PLASMO_PUBLIC_BACKEND_URL || "http://localhost:8080/api",
  timeout: parseInt(process.env.PLASMO_PUBLIC_BACKEND_TIMEOUT || "30000")
})
